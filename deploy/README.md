...
other supported path — an earlier standalone PowerSync-only setup
(assuming an already-existing external Postgres) has been retired in
favor of this one, to avoid two deployment methods drifting out of sync
with each other.

Everything below assumes a bare Linux host with nothing installed yet —
app, api, and the Docker-based backend (Postgres + PowerSync). Already
have Docker and nginx-proxy-manager running? Skip to step 2.
...
`docker-compose.yaml` expects two **already-existing** external Docker
networks — `docker-stack_frontend` / `docker-stack_backend` — so
nginx-proxy-manager can reach `api`/`powersync` by container name. If
...
- `JWT_PRIVATE_KEY` / `JWT_PUBLIC_KEY` — generate a **fresh** keypair for
  this environment (never reuse another deploy's keys):
  ```bash
  npm run -w api generate-jwt-keys
  ```
  paste the two printed lines in as-is (already `\n`-escaped)
- `APP_BASE_URL` / `CORS_ORIGIN` — wherever `app/`'s built static files
  (`app/dist`) end up served from (step 5)
- `SMTP2GO_API_KEY` — optional; unset just logs magic links to
  `docker compose logs api` instead of emailing them, fine for a first
  smoke test
...
PowerSync logs replication starting, no `"level":"error"` lines. This
stack pre-bakes in every real problem hit deploying PowerSync the first
time, back when it was still a separate setup:

- Postgres starts with `wal_level=logical` and `md5` auth already set —
  both are the exact things that bit the original deploy (a
  scram-sha-256 client-library incompatibility, and a restart-required
  WAL setting).
...
- `PS_JWKS_URL` defaults to the internal `http://api:3002/...` address,
  ...
  no way to reach it internally either, back when the two were deployed
  as separate, unrelated stacks.)
...
Not needed at all if `app/` is configured to reach them by internal
container address instead.
...
`docker compose logs api` (or a real email if `SMTP2GO_API_KEY` is set),
click through, and confirm PowerSync sync connects with no 401s in
