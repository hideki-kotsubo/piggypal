# 31 — Home Entry Zone Docked to the Bottom (Trial)

## Status: trial, not decided
...
composing-state variants) — this is for hands-on evaluation, not a locked
replacement of docs/07's entry-zone placement (D22-26). If it's kept, this
doc graduates into a real numbered decision the way docs/19-23 did for the
earlier entry-UX rework; if not, the branch is discarded.
...
- Whether the preview genuinely covering Recent rows (not just docs/07's
  original "still see your data" framing, now literally obscured while
  composing) feels acceptable in real use, not just in a screenshot.
