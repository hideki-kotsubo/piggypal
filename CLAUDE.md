...
- `docs/31-home-bottom-entry-trial.md` — **trial, not decided**, branch `home-bottom-entry-overlay` only: entry zone docked to the bottom of Home instead of the top, overlay-style (position:fixed, no scrim, list never reflows but gets covered while composing). Built 2026-08-19 for hands-on evaluation after the Home-directions design exploration below.
