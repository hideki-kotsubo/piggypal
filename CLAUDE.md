...
`PairingScreen.tsx`) — a genuine WebRTC connection directly between two
browsers, no server involved at all, distinct from and unrelated to the
still-unbuilt PowerSync/API sync boundary above. It doesn't yet move any
real transaction/account data (docs/00-backlog has the exact verified-vs-
deferred breakdown).
...
- `docs/25-p2p-device-sync.md` — new transport, independent of PowerSync: WebRTC data channel + QR-code signaling (serverless, works fully offline), manual both-sides-acked sync sessions. Available to every tier, not a free-tier fallback — paid households can use it too as a fast/offline supplement to PowerSync. QR/SDP size risk resolved by spike (D117a). D125-D127 (added 2026-08-14): pairing asks "your own device, or someone else's?" before anything else — own-device mode unifies identity instead of running the two-person household merge, fixing a gap where solo multi-device sync would've fragmented one person across fake household members. **Transport + UI partially implemented 2026-08-14** (`app/src/lib/pairing.ts`, `PairingScreen.tsx`) — real WebRTC handshake + QR generation/scanning, verified working; D125-D127's identity unification and docs/24's actual data merge are not wired up yet, so "Synced" today means the connection is real, not that data moved.
...
- `docs/27-p2p-pairing-ui-sketch.md` — sketch of docs/25's pairing flow: Settings entry point, the own-device-vs-someone-else fork as two full-screen cards, both sides of the QR exchange, the own-device merge prompt (mirrors docs/05 D14), and the both-sides-acked confirmed state (D128-D131). **Implemented 2026-08-14** (`PairingScreen.tsx`) for frames 2-5, entry point in `SettingsScreen.tsx`; frame 4's merge prompt not wired up yet. Relay-assisted remote signaling and peer rename/forget management stay out of scope, matching docs/25.
